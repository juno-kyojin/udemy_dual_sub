// ============================================================================
// Udemy Dual Subtitles - Background Script (v5 - With Timeouts)
// ============================================================================
...
// API Timeouts
const API_TIMEOUT_MS = 8000; // 8 seconds for external APIs
const CHUNK_SIZE = 20; // Reduced from 30 to avoid rate limiting

// MyMemory API endpoint (free, no API key required)
const MYMEMORY_API = 'https://api.mymemory.translated.net/get';

/**
 * Wraps a fetch with timeout
 */
...
async function translateWithMyMemory(texts, targetLang = 'vi') {
    ...
    try {
        ...
        console.error('[UDS] MyMemory translation error:', error.message);
        return null;
    }
}
...
async function translateWithGoogle(texts, targetLang = 'vi') {
    ...
    try {
        ...
        console.error('[UDS] Google translation error:', error.message);
    }
    return null;
}
...
async function processSubtitleFile(url, targetLang = 'vi') {
    ...
    try {
        ...
        // Process in chunks using global CHUNK_SIZE
        ...
        console.error(`[UDS] Failed to process subtitle file: ${url}`, e.message);
...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    ...
    if (request.action === 'preprocessTexts') {
        if (Array.isArray(request.texts)) {
            ...
            // Translate in chunks using global CHUNK_SIZE
            (async () => {
